...
let score = 0;
let correctIndex = 0;
let currentCats: CatImage[] = [];

// DOM elemek
const imgElements = Array.from(document.querySelectorAll<HTMLImageElement>(".cat-pictures img"));
const breedElement = document.querySelector<HTMLHeadingElement>("#catbreed")!;
const counterElement = document.querySelector<HTMLDivElement>("#counter b")!;
const starterButton = document.querySelector<HTMLButtonElement>("#starter")!;
...
function updateBreed(name: string) {
...
function setupRound(cats: CatImage[]) {
  currentCats = [...cats];
